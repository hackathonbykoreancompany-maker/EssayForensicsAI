    ...
    <div className="surface-card rounded-2xl p-5 space-y-3">
      <div className="flex items-center justify-between pb-2 border-b border-slate-800/80">
        ...
        <span className="text-[10px] text-slate-400 font-mono">
          {signals.length} {signals.length === 1 ? "signal" : "signals"} flagged
        </span>
      </div>
      ...
      <div className="space-y-2.5">
        {signals.map((signal, i) => (
          ...
          >
            {/* Number Pill */}
            <span className="flex-shrink-0 w-6 h-6 rounded-md bg-slate-800 border border-slate-700 flex items-center justify-center text-slate-300 text-[11px] font-bold font-mono">
              {i + 1}
            </span>
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center justify-between gap-2 mb-0.5">
                ...
                >
                  {signal.severity === "high" ? "Strong Marker" : "Moderate Marker"}
                </span>
              </div>
