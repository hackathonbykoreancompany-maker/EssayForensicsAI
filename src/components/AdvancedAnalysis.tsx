...
const BUCKET_STYLES: Record<SentenceLengthBucket, string> = {
  short: "bg-stone-800 text-stone-200 border-stone-700",
  medium: "bg-stone-900 text-stone-300 border-stone-800",
...
function TermTable({ terms, emptyMessage }: { terms: RepeatedTerm[]; emptyMessage: string }) {
  if (terms.length === 0) {
    return <p className="text-xs text-stone-400 italic py-1">{emptyMessage}</p>;
  }
  ...
  return (
    <div className="overflow-x-auto">
      <table className="w-full text-xs">
        <thead>
          <tr className="text-left text-stone-400 uppercase tracking-wider border-b border-stone-800">
        ...
        <tbody className="divide-y divide-stone-800">
          {terms.slice(0, 8).map((t) => (
            <tr key={t.term}>
              <td className="py-1.5 font-mono text-stone-200">{t.term}</td>
              <td className="py-1.5 text-right font-medium text-stone-300">{t.count}</td>
...
function StatRow({ label, value, unit }: { label: string; value: string | number; unit?: string }) {
  return (
    <div className="flex items-center justify-between py-2 border-b border-stone-800/60 last:border-b-0">
      <span className="text-[12px] text-stone-400 font-medium">{label}</span>
      <span className="text-[12px] text-stone-100 font-semibold">
        {value}{unit ? ` ${unit}` : ""}
      </span>
    </div>
  );
...
function SubSection({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <div className="space-y-2.5">
      <h4 className="text-[11px] font-bold uppercase tracking-wider text-stone-300 pb-1 border-b border-white/10">
        {title}
      </h4>
      {children}
    </div>
...
export default function AdvancedAnalysis({ result }: AdvancedAnalysisProps) {
  ...
  const breakdownItems = [
    { name: "Syntactic Complexity", value: score.breakdown.sentenceComplexityScore, max: 30, color: "bg-stone-300" },
    { name: "Sentence Burstiness", value: score.breakdown.burstinessScore, max: 20, color: "bg-stone-400" },
    { name: "Lexical Diversity (MATTR)", value: score.breakdown.lexicalDiversityScore, max: 20, color: "bg-stone-500" },
    { name: "Length Variance", value: score.breakdown.sentenceLengthScore, max: 15, color: "bg-amber-600" },
    { name: "Cadence Uniformity", value: score.breakdown.sentenceRhythmScore, max: 15, color: "bg-red-500" },
  ];
  ...
  return (
    <div className="surface-card rounded-2xl overflow-hidden">
      ...
      >
        ...
        <svg
          className={`w-5 h-5 text-stone-400 transition-transform duration-200 ${isOpen ? "rotate-90" : ""}`}
          fill="none"
          viewBox="0 0 24 24"
          stroke="currentColor"
          strokeWidth="2"
        ...
        <div className="px-5 pb-5 space-y-6 border-t border-white/10 pt-4">
          ...
          <SubSection title="1. Sentence Length Distribution">
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-2.5">
              {[
                ...
                <div key={stat.label} className="p-2.5 rounded-lg bg-stone-900 border border-stone-800 text-center">
                  <span className="text-[10px] text-stone-400 font-medium uppercase tracking-wider block">{stat.label}</span>
                  <span className="text-base font-bold text-stone-100 leading-none mt-1 block">{stat.value}</span>
                  <span className="text-[10px] text-stone-400">{stat.unit}</span>
                </div>
              ))}
            </div>
            {sentenceLength.lengths.length > 0 && (
              <div className="mt-2">
                <p className="text-[10px] text-stone-400 mb-1.5 font-medium">Per-sentence word count sequence</p>
                <div className="flex flex-wrap gap-1.5 p-2 rounded-lg bg-stone-900 border border-stone-800">
                  {sentenceLength.lengths.map((len, i) => (
                    <span key={i} className="text-[10px] font-mono px-1.5 py-0.5 rounded bg-stone-950 border border-stone-800 text-stone-300">
                      {len}
                    </span>
                  ))}
                </div>
              ...
              <div className="mt-2">
                <div className="flex items-center justify-between text-[10px] text-stone-400 mb-1.5">
                  ...
                  <div className="flex gap-2 font-mono text-[10px]">
                    <span className="text-stone-300">&le;10w (Short)</span>
                    <span className="text-stone-400">11-24w (Medium)</span>
                    <span className="text-amber-300">&ge;25w (Long)</span>
                  </div>
                </div>
                <div className="flex flex-wrap gap-1 p-2 rounded-lg bg-stone-900 border border-stone-800 max-h-20 overflow-y-auto">
          ...
          <SubSection title="3. Vocabulary &amp; Phrase Repetition">
            <div className="space-y-3">
              <div>
                <p className="text-[10px] font-semibold text-stone-300 uppercase tracking-wider mb-1.5">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <p className="text-[10px] font-semibold text-stone-400 uppercase tracking-wider mb-1">Repeated Bigrams</p>
                  <TermTable terms={repetition.repeatedBigrams} emptyMessage="None" />
                </div>
                <div>
                  <p className="text-[10px] font-semibold text-stone-400 uppercase tracking-wider mb-1">Repeated Trigrams</p>
          ...
          <SubSection title="4. Burstiness &amp; Lexical Diversity">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="space-y-1">
                <p className="text-[10px] font-bold text-stone-400 uppercase tracking-wider">Burstiness (Fano Factor)</p>
              ...
              <div className="space-y-1">
                <p className="text-[10px] font-bold text-stone-400 uppercase tracking-wider">MATTR Vocabulary Richness</p>
            ...
            <div className="space-y-2">
              {breakdownItems.map((item) => (
                <div key={item.name} className="flex items-center gap-3">
                  <span className="text-[11px] text-stone-300 font-medium w-40 flex-shrink-0">{item.name}</span>
                  <div className="flex-1 h-2 rounded-full bg-stone-900 overflow-hidden border border-stone-800">
                    <div
                      className={`h-full rounded-full ${item.color}`}
                      style={{ width: `${(item.value / item.max) * 100}%` }}
                    />
                  </div>
                  <span className="text-xs font-semibold text-stone-100 w-10 text-right">
                    +{item.value}
                  </span>
                </div>
              ))}
              <div className="flex items-center justify-between pt-2 border-t border-stone-800 mt-1">
                <span className="text-xs font-bold text-stone-300">Composite Score</span>
                <span className="text-sm font-extrabold text-stone-100">{score.overallScore}%</span>
              </div>
            </div>
          </SubSection>
        </div>
